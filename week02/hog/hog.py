...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    # if 100 <= opponent_score <= 999:
        # opp_ten = (opponent_score // 10) % 10
    # elif opponent_score < 100:
        # opp_ten: int = opponent_score // 10
    cur_one: int = player_score % 10
    opp_ten: int = (opponent_score // 10) % 10
...
def num_factors(n):
    ...
    "*** YOUR CODE HERE ***"
    if n == 1: return 1
    if is_prime(n): return 2 # factors of a prime is 1 and itself

    count = 2

    for i in range(2, int(n**0.5 +1)): # if a factor of n is larger than sqrt(n), there must be a factor smaller than sqrt(n)
        if n % i == 0:
            count += 1
            if i != n // i:
                count += 1
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    result = take_turn(num_rolls, player_score, opponent_score, dice)
    new_score = sus_points(player_score + result)
    return new_score
    # END PROBLEM 4
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    b = sus_update(1, score, opponent_score)
    c = sus_update(2, score, opponent_score)
    if a >= GOAL:
        return 0
    elif b >= GOAL:
        return 1
        ...
        return 2
    avg = make_averaged(roll_dice, 100)
    s = avg(6, six_sided)
    d = a - score
    if d > s:
        return 0
